...
# Function to generate transformed feature names
def transformed_name(key):
    return key + "_xf"

# Function to read TFRecord files with GZIP compression
...
def model_builder():
    # Input layer with transformed feature name
...
# Function to prepare the model for serving
...
def run_fn(fn_args: FnArgs):
    ...
    for example in train_set.take(1):
        features, _ = example
        raw_text = features[transformed_name(FEATURE_KEY)]
        vectorize_layer.adapt(raw_text.numpy())
